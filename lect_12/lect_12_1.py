...
async def get_url(client_session, event_loop, url):
    async with client_session(loop=event_loop) as session:
...
def run_loop():
    ...
    result = loop.run_until_complete(get_url(session, loop, URL))
    return result


def search_of_data(result_of_session):
    root = html.fromstring(result_of_session)
...
if __name__ == '__main__':
    write_to_xls(search_of_data(run_loop()))
